            ...
            <form action="{{ route('admin.mutasi.update', $mutasi) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group mb-4">
                    ...
                    <select class="form-control @error('divisi_tujuan') is-invalid @enderror" id="divisi_tujuan"
                        ...
                        <option value="IT Support CM Sako" {{ old('divisi_tujuan', $mutasi->divisi_tujuan) == 'IT
                            Support CM Sako' ? 'selected' : '' }}>IT Support CM Sako</option>
...
<script>
    function addBarang() {
        ...
        newBarang.innerHTML = `
            <button type="button" class="btn btn-danger btn position-absolute top-0 end-0 m-2" onclick="removeBarang(this)">
                <i class="material-icons opacity-10">delete</i>
            </button>
            <div class="form-group mb-3">
                <label for="nama_barang_${index}" class="form-label">Nama Barang</label>
